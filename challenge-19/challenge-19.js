(function(){
  ...
  */
  ...
  function hasClass(markup, cssClass) {
    var regex = new RegExp(' class=(\'|")(.+)?(\\b' + cssClass + '\\b)(.+)?\\1', 'gm');
    return regex.test(markup);
  }

  var classesToTest = [ 'container', 'text', 'tex', 'date', 'excerpt', 'main' ];

  classesToTest.forEach(function(classToTest) {
    console.log( hasClass(markup, classToTest) + ' para a classe ' + classToTest );
  });
})();